...
for j =0,399,1 do
	local feat_train=torch.CudaTensor(50,ndims)
	for i=1,50,1 do
	    xlua.progress((i+j*50), sz)
	    r=math.min(sz,i+batch_size-1)
	    ims=torch.CudaTensor(1,3,224,224)
	    --for j=1,r-i+1 do
	    --    ims[j]=loadim(train_list[i+j-1]):cuda()
		--	--print(string.format('i: %d, r: %d, j: %d\n', i, r, j))
	    --end
		print(string.format('i: %d, j: %d\n', i, j))
		ims[1]=loadim(train_list[i+j*50]):cuda()
	    net:forward(ims)
	    feat_train[{{i,i},{}}]=net.modules[43].output:clone()
	end
...
print(string.format('processing %d images...',sz))
for j =1,200,1 do
	local feat_train=torch.CudaTensor(100,ndims)
	for i=1,100,1 do
	    xlua.progress((i+j*100), sz)
	    r=math.min(sz,i+batch_size-1)
	    ims=torch.CudaTensor(1,3,224,224)
	    --for j=1,r-i+1 do
	    --    ims[j]=loadim(train_list[i+j-1]):cuda()
		--	--print(string.format('i: %d, r: %d, j: %d\n', i, r, j))
	    --end
		ims[0]=loadim(train_list[(i+j*100)-1]):cuda()
	    net:forward(ims[0])
		print(string.format('i: %d, r: %d\n', i, r))
	    feat_train[{{i,i},{}}]=net.modules[43].output:clone()
	end
	local train_h5_file = hdf5.open(opt.out_name, 'a')
	train_h5_file:write('/images_test', feat_train:float())
	train_h5_file:close()
	collectgarbage()
end
